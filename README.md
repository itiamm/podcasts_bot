...
- 音频保留：30 天后从 R2 删除
- 定时：每天 08:00
- 已验证文本模型：NVIDIA `minimaxai/minimax-m3`
...
| `TEXT_PROVIDER` | 文本改写 provider，支持 `nvidia`、`zhipu`、`dashscope` |
...
| 配置项 | 用途 |
| --- | --- |
| `YTDLP_COOKIES_FROM_BROWSER` | 从浏览器读取 YouTube cookies，常用值 `chrome` |
...
- 推荐混合方案：本地 Whisper 转写，NVIDIA MiniMax-M3 生成中文播客稿，阿里百炼 Sambert 合成音频。
